...
2. **Plain existing hook** — *Given* a hand-written `.git/hooks/pre-commit`, *when* init installs, *then* the gate is installed as `.git/hooks/pre-commit.zto` and a **guarded invocation line** is appended to the existing hook — the user's hook still runs, and so does the gate. The original is never discarded.
3. **husky** — *Given* a `.husky/` directory, *when* init installs, *then* the gate invocation is added to `.husky/pre-commit` (created if absent) and `.git/hooks/` is **not** touched directly.
4. **lefthook** — *Given* a `lefthook.yml` (or equivalent), *when* init installs, *then* the gate is registered so lefthook runs it on pre-commit (or, if a safe automated edit isn't possible, init reports the exact snippet to add — see clarify).
...
- **FR-001 — Hook-situation detection.** Before installing, classify the target into one of: **none** (no `pre-commit`, no manager), **plain** (a non-framework `.git/hooks/pre-commit` exists), **husky** (`.husky/` present), **lefthook** (`lefthook.{yml,yaml,toml,json}` or a `lefthook` key in `package.json`). A hook that is already the framework gate (identified by its marker line) counts as **none**/already-installed, not plain.
...
- **FR-003 — Chain a plain hook.** With a plain existing `.git/hooks/pre-commit`, install the gate as `.git/hooks/pre-commit.zto` (executable) and append a **guarded invocation line** to the existing hook so both run. The user's script is **modified only by appending** — never truncated, reordered, or replaced.
- **FR-004 — husky integration.** With husky detected, add the guarded gate invocation to `.husky/pre-commit` (creating it if absent), and **do not** write `.git/hooks/pre-commit` (husky owns it).
- **FR-005 — lefthook integration.** With lefthook detected, wire the gate so lefthook runs it on `pre-commit` — either by registering a command in the lefthook config, or (per clarify) by reporting the exact config snippet for the user to add. Never silently rewrite the user's lefthook config in a lossy way.
...
- **Guard marker** — a stable comment/line (e.g. `# zero-two-one gate`) wrapping every chained invocation, so idempotency is a simple presence check and the user can find/remove it.
...
- Plain hook → original content preserved verbatim + a guarded invocation appended; `.git/hooks/pre-commit.zto` present and executable; both run.
- husky → `.husky/pre-commit` carries the guarded invocation; `.git/hooks/pre-commit` untouched.
- lefthook → the chosen strategy (register or report) leaves the user's config non-lossy and wires or documents the gate.
...
*Deferred to clarify: whether lefthook wiring is **automated** (append a command to the config) or **report-only** (print the snippet, don't edit the user's YAML); the **chain order** (gate before or after the user's existing hook/commands); and which **husky layout** to target (`.husky/pre-commit` v9+ vs the older `.husky/_/husky.sh`). None change the spec's shape.*
