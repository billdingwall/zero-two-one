...
| [006-source-layer-renderer](006-source-layer-renderer/spec.md) | Source Layer & Stack-Parameterized Renderer — neutral `ASSISTANT-Template.md` source + zero-dep renderer; `classes.js`/`sources.js` resolve the install surface from `tools.stack`; `claude` byte-identical, `antigravity` `AGENTS.md` entrypoint (TDD §9.1, r9) | mvp-4 | Draft |
