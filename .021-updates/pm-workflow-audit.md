...
| `04-ROADMAP.md` | Sequences the delivery milestones and phases. | Product Management |
| `05-BACKLOG.md` | Captures and prioritizes refinement tasks, features, bugs. | Product Management |
...
flowchart TD
    subgraph Phase 1 & 2: Planning & Pre-Build
        ...
        B --> K[Prioritize in 05-BACKLOG.md]
        C --> D[Review & Iterate]
        D --> E{MVP Aligned?}
        E -- No --> B
    end
...
*   **Tracking Artifact:** The `04-PROJECT-TRACKING.md` was intended for pre-MVP tasks. We need a persistent `05-BACKLOG.md` to track refinement tasks, feature enhancements, and bugs across the entire lifecycle.
...
### High Priority
*   **Formalize `05-BACKLOG.md`:** Rename and reposition the tracking document to `requirements/05-BACKLOG.md` as the universal intake for tasks across all lifecycle phases.
...
### Medium Priority
*   **Backlog Context Generation:** Update the SpecKit context builder (`scripts/speckit/fetch-speckit-context.js`) to ingest `05-BACKLOG.md` alongside the living PRD when generating feature specs.

### Low Priority
*   **Roadmap Automation:** Link the `04-ROADMAP.md` more tightly to GitHub milestones so that SpecKit completion automatically updates the roadmap artifact.
...
*   **Existing Legacy Projects:** We need a "Baseline Extraction" AI skill that reads an existing product and reverse-engineers a "Living PRD" and an initial `05-BACKLOG.md`, allowing them to adopt the Refinement Loop natively.
