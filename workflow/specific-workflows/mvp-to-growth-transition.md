...
The MVP Build exit gate passes: the MVP is launched and the QA suite is green (see the [Product Lifecycle](product-lifecycle.md)).

## Transition Steps

1. **Freeze the MVP Releases.**
   In `05-ROADMAP.md`, the MVP Releases section becomes a historical record of how the product got here: check off completed items, close the release headers, write the **Delivered** summary in each `requirements/_releases/mvp-N.md` file, and add no new scope there.

2. **Activate the Releases section.**
   The `Releases (Growth)` section at the top of `05-ROADMAP.md` becomes the active planning surface. Upcoming work is expressed as named releases (`Release v1.x — <theme>`), each **tied to a specific release branch**, with a dedicated file in `requirements/_releases/` (from `templates/10-RELEASE-Template.md`) created at release open and a Delivered summary written at close. The roadmap keeps the summary and link; detail lives in the release file.

3. **Flip the backlog.**
   Move all remaining v2/deferred items into the `v2 / Growth Backlog` in `04-BACKLOG.md`. Prioritization switches from roadmap-driven to **user value**, defined from user feedback collected through refinement rounds and `021-feedback` issues.
