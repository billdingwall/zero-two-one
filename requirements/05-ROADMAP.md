...
| [mvp-4](_releases/mvp-4.md) | AI-Led Init & Stack/Design Adapters — three-layer model (neutral core + per-stack adapters at init); spec cut 006 source-layer/stack-parameterized renderer · 007 Antigravity · 008 Kiro + engine dispatch · 009 `021` CLI; neutral-core invariant; API-surface decision (r9) | ✅ Delivered (adapter cut 006–009, 2026-07-18) — AI-led walkthrough + init-time `material-3` auto-binding **descoped to backlog** (2026-07-19) | High | mvp-3 | MVP Build |
| [mvp-5](_releases/mvp-5.md) | Lifecycle Commands — `021-feedback`, `021-design`, `021-prototype`, stage-specific review-template selection | ✅ Delivered (specs 010–012 + review-template wiring, 2026-07-19) | Medium | mvp-4 | MVP Build |
| [mvp-6](_releases/mvp-6.md) | Test & Publish — e2e across all stacks/modes (**spec 013 ✅**), CI provenance publish pipeline + pre-publish gate, then **publish v1.1.x to npm** so the framework is installable in real repos | ◧ In Progress | High | mvp-5 | MVP Build |
| [mvp-7](_releases/mvp-7.md) | Field Test & Review — install the **published** package into three real repos (per stack/phase), live feedback into the backlog, then a full-work review across all MVP releases; **the terminal MVP release and the MVP→Growth gate** | ◻ Planned | Medium | mvp-6 | MVP Build |
...
When the **mvp-7** exit gate passes (published package field-tested in real repos, feedback arriving, full-work review clean, QA green), the roadmap changes shape: the MVP Releases above freeze as history and the Releases (Growth) section takes over, pulling from the backlog. The **v2 feature set is defined at that point** (team direction, r5). Mechanics: [mvp-to-growth-transition.md](../workflow/specific-workflows/mvp-to-growth-transition.md). *(Publishing itself happens earlier, in mvp-6, to enable the field test — it is no longer the transition trigger.)*

## Changelog
- **2026-07-20:** **Split mvp-6 → mvp-6 (Test & Publish) + mvp-7 (Field Test & Review).** Publishing moved **into** mvp-6 (no longer the terminal act, reversing r8) so `zero-two-one` can be installed and tested in real repos; the field test (now against the **published** package), feedback loop, and full-work review moved to the new **mvp-7**, which becomes the terminal MVP release and the MVP→Growth gate. Rationale: mvp-3 safe-install is Delivered and **spec 013's e2e** proves the packed tarball installs cleanly across all three stacks × both modes, resolving the r5/r8 reason to gate publish behind review. mvp-6 step 1 (e2e) closed via spec 013; the r9 per-stack fresh-install smoke test is subsumed by it. MVP Build now spans mvp-3…mvp-7.
- **2026-07-19:** mvp-4 closed **Delivered** on the adapter-cut scope (specs 006–009); the AI-led init walkthrough and init-time `material-3` auto-binding descoped to `04-BACKLOG.md` (the binding *mechanism* had already shipped independently via `021-design`, mvp-5). See [_releases/mvp-4.md](_releases/mvp-4.md) changelog.
- **2026-07-16 (r9):** mvp-4 row re-described around the three-layer model + spec cut (006 source-layer/renderer · 007 Antigravity · 008 Kiro + dispatch · 009 `021` CLI) and the neutral-core invariant. Per [_refinement/r9-review.md](_refinement/r9-review.md) (source: [_notes/repo-refactor.md](_notes/repo-refactor.md)).
