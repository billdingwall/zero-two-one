...
**r3 designed (2026-07-10):** three supported stacks locked in TDD §9 (`claude` + Spec Kit · `antigravity` + Spec Kit · `kiro` all-in-one) plus the independent design-system role (`none` / `material-3`) and the `021-` naming convention. Implementation is staged in the v2 backlog as the "Stacks & Design Adapters" group, pulled into Growth releases (or earlier at the team's discretion).
