...
| **Workflow Manager (TDD §13)** — post-commit/assistant-side state-sync; advisory, never blocks/auto-commits | Open | Eng | mvp-3 |
...
| **Acceptance matrix: 3 stacks × {none, material-3}** — gate green + `021-status`/`021-qa` in all six cells | Open | Eng | mvp-4 |
...
| Automated tests for `bin/init.js` and `hooks/pre-commit` | Open | Eng | mvp-6 |
| End-to-end test via Claude Code — scaffold and migrate modes | Open | Eng | mvp-6 |
| Publish `zero-two-one` v1.1.x to the NPM registry (**after** mvp-3 safe-install lands) | Open | Eng | mvp-6 |
