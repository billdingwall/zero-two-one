...
- **r5** (2026-07-12, Applied): [r5-review.md](r5-review.md) — `/harden-docs` alignment audit + team answers. Six engineering-ordered MVP releases (mvp-1…mvp-6); dogfooded `.zero-two-one.json` + `workflow-status.js` manifest-read (phase drift resolved); optional `021-prototype` (prototype off the critical path); publish gated behind safe-install; v2/Growth backlog emptied; new `requirements/_design/command-design.md` + `workflow-design.md`; feedback repo slug resolved; success metrics reframed.
