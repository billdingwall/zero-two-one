...
2. [x] **CI publish pipeline built** (r7, TDD §14): tag-triggered, `npm run sync:package -- --check` then `npm publish --provenance` from `package/`; **pre-publish gate** fails on a dangling `main`, missing `LICENSE`, `.ai/context` dummies in the tarball, broken links, or any internal feature spec (`specs/00N-*`) / dev file in the tarball (the spec 001–005 P1 regression check, r9). Plus an **install-focused shipped README** (split from the repo/contributor README). Delivered as **[spec 014](../../specs/014-publish-pipeline/spec.md)** (Done): `scripts/prepublish-gate.js` + `.github/workflows/publish.yml` (trusted publishing, OIDC) + separate `package/README.md`. *(The r9 "fresh-install smoke test per stack" is **subsumed by spec 013's e2e**, which already installs the packed tarball per stack — this release keeps only the tarball-content audit, now enforced by the gate.)* Manual `publish:package` is a documented fallback only (now gated).
3. [ ] **Publish `zero-two-one` v1.1.x to npm** — fire the pipeline (name verified unclaimed — npm 404, r7). **Maintainer action** (credentialed, irreversible): (a) configure a trusted publisher for `zero-two-one` on npmjs.com (repo `billdingwall/zero-two-one`, workflow `publish.yml`), or seed once via `npm run publish:package`; (b) `git tag v1.1.0 && git push --tags` to fire the pipeline. Repo is release-ready (versions 1.1.0 consistent, gate PASS, sync clean, 124 tests green). This makes the framework installable via `npx zero-two-one-init` / `npx 021 …` in any repo, which is what mvp-7's field test consumes.
