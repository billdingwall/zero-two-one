...
4. **Spec-Driven Delivery (SSD)**
   Items are pulled from the backlog into the SSD pipeline (SSD feature branches, `NNN-feature-name`). The `.ai/context/` generation script automatically bundles `CODE.md`, `PRODUCT.md`, and `05-BACKLOG.md` to ensure the AI agent understands the implementation requirements within the context of the living architecture.
