...
- **FR-002 — Phase heuristics.** Migrate infers a likely lifecycle phase with a **strict precedence** *(clarified 2026-07-16)*: `growth` requires **all** of tests + CI + release history (**git tags**, clarified); failing that, substantial code (**≥1 source file outside config/docs/tests dirs**, analyze A2) but no framework key docs ⇒ `mvp`; otherwise ⇒ `planning`. The inference and its evidence are surfaced, never silently applied.
...
- **FR-009 — Growth entry.** When the confirmed phase is `growth`, migrate scaffolds `05-ROADMAP.md`/`04-BACKLOG.md` in **post-transition shape** (Releases section active, MVP section frozen as history) per [mvp-to-growth-transition.md](../../workflow/specific-workflows/mvp-to-growth-transition.md). When the user already has these files (exact-dest collisions), **duplicate resolution (FR-007) governs** — growth-entry only selects the post-transition template *variant* used for a create/archive/update, and never overrides the user's file *(clarified via analyze A1)*.
...
- **Migrate decision record** — the manifest fields this feature adds/sets: `mode: migrate`, confirmed `phase`/`tools`, and a `migrate` block — `duplicates` (per path → archive|update|leave), `imported` (leave-alongside paths, keyed for idempotency), and `archived` (dest → archive path). Makes the adoption auditable and re-runs manifest-driven.
...
- When phase is `growth` and the user already has `05-ROADMAP.md`/`04-BACKLOG.md`, **duplicate resolution governs** — growth-entry does not overwrite them.
- `--design material-3` is recorded as `tools.design` in the manifest (installation stays `021-design`/mvp-4).
