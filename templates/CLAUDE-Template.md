...
## AI Instructions
- Always rely on your internal memory first to understand the current phase and state of the project. Verify with `npm run status`. When a `.zero-two-one.json` manifest is present at the repo root, read it to learn the lifecycle phase and tool stack instead of inferring from directory contents.
