...
2. **Pre-build (Refinement)**: Install Github Speckit. Establish an Experience Design Document (`EDD.md`) and build a static prototype using the first spec. Enter a refinement loop (review -> synthesize -> update docs & prototype) until the project reaches the desired fidelity.
